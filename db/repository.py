...
class Repository(Generic[T]): #Generic
    def __init__(self):
         name = get_args(self.__orig_bases__[0] )[0].__name__.lower().replace('model','') #con esto pasamos de llamarlo "Mesamodel" a "mesa"
...
#Metodos
    ...
    #filtro            
    def query(self, filter): #recibe filtro
        results = self.collection.find(filter) #print filtro
        data = []
        for r in results:
            r["_id"] = r["_id"].__str__()
